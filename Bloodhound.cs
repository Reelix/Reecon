...
namespace Reecon
{
    public class Bloodhound
    {
        ...
        public static void GetInfo(string userId)
        {
            ...
            string profileId = JsonDocument.Parse(profileIdReq.Text).RootElement.GetProperty("data")[0]
                .GetProperty("objectid").ToString();
            Console.WriteLine("PID: " + profileId);

            (string Text, HttpStatusCode StatusCode) ownablesReq =
                Web.DownloadString("http://localhost:8080/api/v2/users/" + profileId + "/controllables?type=graph",
                    JWT: jwt);
            ...
            foreach (NodeItem node in parsedNodes)
            {
                // {node.Data?.Name}
                Console.WriteLine(
                    $"  Node Key: {node.OriginalKey}, Name: {node.Data?.Name} (ObjectId: {node.Data?.ObjectId}), Type: {node.Data?.NodeType}");
            }
            ...
            {
                Console.WriteLine(
                    $"  Rel Key: {rel.OriginalKey}, Type: {rel.LabelInfo?.Text}, From: {rel.Id1}, To: {rel.Id2}");
            }
            ...
            NodeItem myNode = parsedNodes.First(x => x.Data?.ObjectId == profileId);
            Console.WriteLine("Relationships");
            Console.Write(myNode.Data?.Name + " (" + myNode.Data?.NodeType + ") ");

            // Now the fun part
            List<NodeItem> sortedNodes = [];
            List<RelationshipItem> sortedRelationships = [];
            sortedNodes.Add(myNode);
            while (true)
            {
                List<RelationshipItem> connectedNodes =
                    parsedRelationships.Where(x => x.Id1 == myNode.OriginalKey).ToList();
                if (connectedNodes.Count == 0)
                {
                    break;
                }

                foreach (RelationshipItem rel in connectedNodes)
                {
                    string? connectedNodeId = rel.Id2;
                    myNode = parsedNodes.First(x => x.OriginalKey == connectedNodeId);
                    sortedNodes.Add(myNode);
                    sortedRelationships.Add(rel);
                    Console.Write("-> " + rel.LabelInfo?.Text + " -> " + myNode.Data?.Name + " (" +
                                  myNode.Data?.NodeType + ") ");
                }
            }
            ...
            if (sortedNodes.Count == 2 && sortedRelationships.Count == 1)
            {
                    ...
                    sortedRelationships[0].LabelInfo?.Text == "WriteSPN")
                {
                    ...
                    Console.WriteLine("- Interesting Thing Found!");
                    Console.WriteLine($"- {originUser} has the ability to write to the 'serviceprincipalname' of {kerberoastableUser} so you can do a targeted kerberoast attack against them.");
                    ...
                    Console.WriteLine($"-- python3 targetedKerberoast.py -v -d '{domain}' -u '{originUser}' -p 'PASSWORD_HERE'");
            ...
            if (sortedNodes.Count == 2 && sortedRelationships.Count == 1)
            {
                    ...
                    sortedRelationships[0].LabelInfo?.Text == "AddSelf")
                {
                    ...
                    Console.WriteLine($"-- bloodyAD --host {"IP".Recolor(Color.Green)} -d {userDomain} -u {userName} -p '{"PASSWORD".Recolor(Color.Green)}' add groupMember {groupName} {userName}");
                }
            }
            ...
            if (sortedNodes.Count == 3 && sortedRelationships.Count == 2)
            {
                    ...
                    sortedRelationships[1].LabelInfo?.Text == "GenericAll")
                {
                    ...
                    string? groupName = groupNode.Data?.Name?.Split('@')[0];
                    Console.WriteLine(
                        $"-- Check if user is a member of the group: net rpc group members \"{groupName}\" -U \"{username}\"%\"PASSWORD\" -S \"{userDomain}\"");
                    Console.WriteLine(
                        $"-- Add user to group: net rpc group addmem \"{groupName}\" \"{username}\" -U \"{username}\"%\"PASSWORD\" -S \"{userDomain}\"");
                }
            }
                ...
                sortedNodes[0].Data?.NodeType == "Group" && sortedNodes[1].Data?.NodeType == "User")
            {
                ...
                if (userNodes.Count == sortedNodes.Count - 1)
                {
                    foreach (NodeItem userNode in userNodes)
                    {
                        ...
                        {
                            string? username = userNode.Data?.Name?.Split('@')[0];
                            Console.WriteLine($"- certipy shadow auto -u 'USERNAME' -p 'PASSWORD' -dc-ip IP -account '{username}'");
                            Console.WriteLine($" -- certipy find -u '{username}' -hashes HASH_FROM_ABOVE -dc-ip IP_HERE -text -vulnerable -stdout");
                        }
                    }
                }
            }
        ...
        private static string? Auth()
        {
            ...
            string postData = "{\"login_method\":\"secret\",\"username\":\"admin\",\"secret\":\"Password1!!!\"}";
            Dictionary<string, string> headers = new() { { "Content-Type", "application/json" } };
            byte[] byteData = Encoding.ASCII.GetBytes(postData);
            Web.UploadDataResult authResult = Web.UploadData($"http://localhost:8080/api/v2/login",
                PostContent: byteData, ContentHeaders: headers);
        ...
        private static (List<NodeItem> Nodes, List<RelationshipItem> Relationships) ParseJsonBlob(string jsonBlob)
        {
            ...
            var relationships = new List<RelationshipItem>();

            // Get the default instance of the source-generated context.
            var context = BloodhoundJsonContext.Default;
    ...
    public class NodeDataItem
    {
        ...
        public string? SystemTags { get; set; } // Can be null or a string like "owned"
    }
    ...
    public class NodeItem
    {
        ...
        public NodeLabel? LabelInfo { get; set; } // Renamed to avoid conflict if a property was just 'Label'
