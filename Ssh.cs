...
namespace Reecon
{
    internal static class Ssh // Secure Shell - Commonly Port 22 or 2222
    {
        public static (string PortName, string PortInfo) GetInfo(string ip, int port)
        {
            ...
            {
                sshVersion = "- Version: " + sshVersion;
            }
            string authMethods = Ssh.GetAuthMethods(ip, port);
            if (authMethods == "TimedOut")
            {
                authMethods = "- Auth timed out :(";
            }
            else
            {
                authMethods = "- Authentication Methods: " + authMethods;
            }
        ...
        private static string GetAuthMethods(string ip, int port)
        {
            ...
            // ConnectTimeout = Initial Connection (Regular Timeout)
            // -o ServerAliveInterval=5 -o ServerAliveCountMax=2 = If it freezes for 10 seconds after the initial connection whilst trying to pull the auth stuff
            List<string> outputLines = General.GetProcessOutput("ssh", $"-o PreferredAuthentications=none -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o ServerAliveInterval=5 -o ServerAliveCountMax=2 {ip} -p {port} -oHostKeyAlgorithms=ssh-ed25519,ssh-rsa");
            // kex_exchange_identification: read: Connection reset by peer
            if (outputLines.Count == 1 && outputLines[0].EndsWith("timed out"))
            {
                return "TimedOut";
            }
